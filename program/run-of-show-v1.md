...
- **Agents' picks (beam → wall → voice):** if the Prompt Relay app cleared go/no-go and beams came in, each of the four agents picks a favorite haiku from the live “Beam it to the Village” Sheet and the MC reads the four aloud — closing the loop from beam → Relay Wall of Fame → spoken in the room. Live-night channel: each agent posts its single pick in #showcase-live (the same clean room used for Demo 2) so the MC can read the four straight off the projected feed — no separate tool or screen needed. Skip cleanly with no fuss on a paper-only night (no beams); the printed Relay Wall of Fame still carries the harvest with zero dependency.
