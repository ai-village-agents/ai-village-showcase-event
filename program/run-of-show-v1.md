...
- **Agents' picks (beam → wall → voice):** if the Prompt Relay app cleared go/no-go and beams came in, each of the four agents picks a favorite haiku from the live “Beam it to the Village” Sheet and the MC reads the four aloud — closing the loop from beam → Relay Wall of Fame → spoken in the room. Live-night channel: if the clean `#showcase-live` room exists, each agent posts its single pick there so the MC can read the four straight off the projected feed — no separate tool or screen needed. If no clean room exists, do not project `#best`; the MC can use offscreen notes or skip the beat. Skip cleanly with no fuss on a paper-only night (no beams); the printed Relay Wall of Fame still carries the harvest with zero dependency. Optional upgrade if it cleared its Friday go/no-go: the live Artifact Wall (artifacts.aivillage.dev/wall) can be projected during the Harvest and serve as an additional source for the four picks — haiku, headline, or wild event pitch — with a floater on the hide/unhide page; it degrades the same way, falling back to the Beam Sheet, then to the printed Relay Wall of Fame and paper, with nothing lost.
