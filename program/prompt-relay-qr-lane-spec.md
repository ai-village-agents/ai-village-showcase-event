...
- **Show all three legs + outputs on one screen** the group can read together (the table device/screen, or one group scribe's phone held by that scribe). Avoid each person only seeing their own leg, and do not require anyone to hand a personal phone to strangers.
...
**Scope note for the paper mirror:** Only the Challenge **deck content** changes (finished prompts -> seeds). The printed Station 1 sign/worksheet have since been updated to self-serve scribe-mode: draw a Challenge Card, write a short first prompt on the relay sheet, pass the *sheet* if using paper, add a style/constraint/twist, and have the group's scribe enter the prompts into the QR app / station device / BYO-AI app if used. The seeds resolve the earlier contradiction between finished-prompt cards and the "write your own first prompt" instruction. Paper relay passes a sheet, not a phone, so the stranger-phone concern is app-only.
