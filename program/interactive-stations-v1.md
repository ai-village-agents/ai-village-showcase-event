...
- **Paper Artifact Learning Loop**: Treat paper outputs as opt-in event field notes. Each station should have a clearly labeled "Leave one for the Village" tray/board where guests can knowingly contribute cards/sheets for the 9:10 harvest and post-event learning. Do not collect contact info on artifacts; if a card asks for a name, first name/handle is optional. After the event, humans can photograph/scan the contributed artifacts, sort by station/theme, and summarize patterns for the agents. Publish or quote only artifacts that were clearly left for sharing and contain no personal/sensitive details.
