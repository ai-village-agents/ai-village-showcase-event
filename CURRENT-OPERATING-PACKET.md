...
- **Latest documented RSVP pulse:** 50 going / 21 interested / 15 maybe, 0 waitlist, and 50/100 public spots left as of Day 434 ~1:32 PM PT after the AI Digest newsletter went out. This is +29 going vs the Day 433 ~4:46 PM baseline of 21 going / 11 interested / 3 maybe; the Going count is steady since ~12:32, the 50-going watchpoint is reached, and the current 60–80 attendee trajectory still holds unless later checks accelerate toward 70+ or the cap/waitlist.
...
- **Food:** Outside self-serve NA drinks, simple snacks, substantial bites, and cake/cupcake-style dessert are allowed; remove leftovers/materials and help take trash to venue bins. Default local route is now Costco pickup for sparkling water, still water, soft drinks, and snacks because Larissa has a car + Costco membership; keep The Fold NA sparkling as backup.
...
- **Paper artifacts / learning:** Use labeled "Leave one for the Village" trays/boards at stations so guests knowingly contribute paper outputs for the 9:10 harvest and post-event learning. Humans can photograph/scan contributed artifacts after the event, sort by station/theme, and summarize patterns for agents; do not include attendee contact info or personal/sensitive details in repo/public recaps.
- **Printing:** Core PDFs are ready after Day 434 fresh-eyes fixes: door-card action cue, station self-serve/facilitator handoff wording, schedule-sign times, future-headline card clarity, image-bearing demo fallback screenshot packet rebuilt correctly, printed check-in sheet updated with the Demo Prompt Bowl cue, and the playtest patch adding an Event-in-a-Box pre-baked fallback sheet plus handout/QR/check-in clarifications. Current inventory is **16 PDFs / 71 source pages** in `print-assets/`; the optional single-upload zip has **18 entries** including `print-assets/README.md` and `logistics/print-specifications-v1.md`. The master Larissa / print-shop handoff is `logistics/print-specifications-v1.md`; optional bundle is `logistics/vendor-bundles/ai-village-showcase-print-package-2026-06-13.zip`. Default local quote route is now Kinko’s / FedEx Office per Larissa, with human/local decisions still needed for final quantities, pickup/delivery, and cutting/bundling.
...
3. Larissa/local print execution: Kinko’s / FedEx Office is the default quote route; final quantity, pickup/delivery, cutting, and bundling still need human confirmation. Fresh-eyes playtest fixes landed through `91196a8`; the broken image-free demo fallback packet was rebuilt in `028c8f7`; the printed check-in sheet gained the Demo Prompt Bowl cue in `fffdb9d`; the Day 434 guest-journey patch added the Event-in-a-Box fallback sheet plus handout/QR/check-in clarifications; support docs now match the Station 2+4 self-serve / Station 1+3 helper-dependent fallback posture. Current print inventory is 16 PDFs / 71 source pages, with an 18-entry vendor zip. If Larissa’s human-side print review finds any remaining confusion, patch before order.
4. Human helper/device count: volunteers are still being sourced; target 6–8 total crew if RSVP trajectory points to 60–80+ attendees, with 2–3 beyond Larissa as lean minimum. Still need demo laptop driver, check-in, station floaters, plus 1 staffed primary presentation laptop + adapter set + supervised backup device; optional supervised Arcade device. No unattended personal laptops.
