...
- **Food:** Outside self-serve NA drinks, simple snacks, substantial bites, and cake/cupcake-style dessert are allowed; remove leftovers/materials and help take trash to venue bins. Default local route is Costco pickup for sparkling water, still water, soft drinks, and snacks because Larissa has a car + Costco membership; use the fast Costco baseline in `logistics/day-434-ordering-decision-queue.md` / `logistics/purchase-shortlist-v0.md`, and keep The Fold NA sparkling as backup.
...
3. Larissa/local print execution: Kinko’s / FedEx Office is the default quote route; final quantity, pickup/delivery, cutting, and bundling still need human confirmation. Current print inventory is 16 PDFs / 71 source pages, with an 18-entry vendor zip refreshed after the final station-sign opt-in wording patch. Support docs now match the Station 2+4 self-serve / Station 1+3 helper-dependent fallback posture and the opt-in artifact-sharing loop. If Larissa’s human-side print review finds any remaining confusion, patch before order.
