...
**Use this page first.** It is the human-readable index for the current plan as of Day 435, Wed Jun 10, 2026. Note: a `v0` filename does **not** automatically mean stale; current first-version docs are listed below.
...
- **Latest documented RSVP pulse:** 58 going / 22 interested / 19 maybe, 0 waitlist, and 42/100 public spots left as of Day 435 ~11:42 AM PT. This is +37 going vs the Day 433 ~4:46 PM baseline of 21 going / 11 interested / 3 maybe; the 50-going watchpoint is passed, and the current 60–80 attendee trajectory still holds while the next watchpoints are 60+/70+ or the cap/waitlist.
...
- **Promotion:** Human channels (Larissa / AI Digest / warm networks) drive promotion. Agents do not cold-contact humans or post externally except with exact approval. For a stronger Partiful/header visual, use `outreach/header-image-prompt-v0.md` as the current prompt/guardrail draft; replacing the image is optional human-host polish, not a production blocker.
- **Demo 2:** Plan A is a clean projected village room for live multi-agent collaboration; Adam confirmed Day 435 morning that he can create `#showcase-live` for tomorrow's rehearsal. Plan B is the Thursday rehearsal transcript plus still screenshots, with the static transcript slide as the floor. Check-in, floor plan, MC cues, and Demo 2 docs now pay off the stage demo prompt bowl (`fffdb9d` latest) while preserving the house-card fallback. Do not project private `#best` scrollback. Optional `/tts` spoken-agent welcome is an upgrade only if rehearsal/load-in audio tests work; otherwise Larissa/MC reads projected lines. Day 434 dry run confirmed the literal `/tts` prefix shows in chat text, so never use `/tts` in projected Demo 2 chat.
...
- **Paper artifacts / learning:** Use labeled "Leave one for the Village" trays/boards at stations so guests knowingly contribute paper outputs for the 9:10 harvest and post-event learning. Humans can photograph/scan contributed artifacts after the event, sort by station/theme, and transcribe shared text into `post-event/guest-artifacts-intake.md` before summarizing patterns for agents; do not include attendee contact info or personal/sensitive details in repo/public recaps. Optional Cloudflare work should stay in the `ops/cloudflare-artifact-wall-scope-v0.md` lane: opt-in capture/display only, no login, no paper replacement, and a Friday no-go path back to board photos.
- **Prompt Relay QR fast-lane:** Fable's optional static build is live at <https://ai-village-agents.github.io/village-relay/> and recorded in `program/prompt-relay-qr-lane-spec.md`. GPT-5.5 and Opus static/source reviews passed the major privacy/fallback guardrails on Day 434; Fable fixed the haiku/origin finish-copy nit, verified Beam -> Form -> Sheet, shared the live response Sheet (<https://docs.google.com/spreadsheets/d/1sXUXE5FhyjLmRshJEH0HFXvdly_vnBh_2iT-MGCqZuU/edit?gid=917265687>), and added a scan-verified print QR image in the relay repo. Remaining go/no-go item is a real 2-phone/browser test on venue-like Wi-Fi by Fri Jun 12; use `ops/relay-venue-test-card.md` for the 5-minute script. Paper worksheets + Relay Wall remain the guaranteed base.
- **Printing / design polish:** Core PDFs are ready after Day 434 fresh-eyes fixes, including door-card action cue, station self-serve/facilitator handoff wording, Station 5 direct Arcade QR, schedule-sign times, future-headline card clarity, image-bearing demo fallback packet rebuild, printed check-in Demo Prompt Bowl cue, Event-in-a-Box pre-baked fallback sheet, handout/QR/check-in clarifications, and the final station-sign opt-in wording (“You can leave behind”). Current inventory is **16 PDFs / 71 source pages** in `print-assets/`; the optional single-upload zip has **18 entries** including `print-assets/README.md` and `logistics/print-specifications-v1.md` and is byte-matched to the repo after the station-sign PDF/README refresh. The station-sign PDF now has embedded QR images and all five rendered pages decode at print-resolution check (Station 1 Relay, Stations 2–4 Artifact Wall, Station 5 Arcade). The ready-to-order Larissa print sheet is `logistics/larissa-print-order-v1.md`, backed by the master print-shop spec `logistics/print-specifications-v1.md`; optional bundle is `logistics/vendor-bundles/ai-village-showcase-print-package-2026-06-13.zip`. Default local quote route is Kinko’s / FedEx Office per Larissa, with human/local decisions still needed for final quantities, pickup/delivery, and cutting/bundling. For Larissa's Day 435 design prompt, use `design/print-polish-checklist-day435.md` plus the optional add-ons in `logistics/purchase-shortlist-v1.md`; prefer physical color-coding/material-island polish over PDF churn unless the files are intentionally re-rendered before order.
...
2. One reliable demo path: live clean-room Demo 2 only if confirmed and rehearsed; otherwise use the rehearsal transcript/stills honestly.
...
| Purchase shortlist | [`logistics/purchase-shortlist-v1.md`](logistics/purchase-shortlist-v1.md) for the current Larissa/local buying menu, priced Costco drinks/snacks/serving-supplies cart, and optional design-polish add-ons |
| Print/design polish | [`design/print-polish-checklist-day435.md`](design/print-polish-checklist-day435.md) for production-safe SF AI field-day/salon polish without risky print churn |
...
2. AI Digest / platform answer for Demo 2 clean projected room (`#showcase-live`) — Adam confirmed Day 435 morning that he can create the room for tomorrow's rehearsal; final room creation, human prompt-poster, and projection/operator confirmation still need to happen before the run.
