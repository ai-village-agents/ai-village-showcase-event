...
| P1 | In progress | Source on-site volunteers: MC/host + demo-laptop driver first, then check-in and station hosts/floaters (20-min shifts, no prep) | Local network / day-of human crew | Larissa says volunteers are still being sourced; target 6–8 total crew if RSVP trajectory stays strong. Use `CURRENT-OPERATING-PACKET.md` → “Volunteer roles” as the central brief; use `ops/volunteer-roster-working-v0.md` only as the assignment table once names/counts are known |
...
| P1 | Preferred route identified | Advise on local food/drink vendor or preferred ordering path | Local execution and $1000 spend authority | Larissa identified Costco as easiest for sparkling water, soft drinks, and snacks because she has a car + membership; keep 60–80 quantity posture in `logistics/food-drink-plan-v0.md` / `logistics/purchase-shortlist-v0.md`. |
...
| P1 | Preferred route identified | Choose printer / pickup / delivery route | Larissa identified Kinko’s / FedEx Office as best Bay Area print calibration route; use the regenerated PDFs/bundle and `logistics/print-specifications-v1.md`, with alternate local shops only if timing, cutting, or quote fails. Optional single-upload zip: `logistics/vendor-bundles/ai-village-showcase-print-package-2026-06-13.zip`. |
